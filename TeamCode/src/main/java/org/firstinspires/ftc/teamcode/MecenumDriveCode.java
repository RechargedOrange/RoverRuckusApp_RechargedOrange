 ...
 */
@TeleOp (name = "MecanumDriveCode")

public class MecenumDriveCode extends superClass {
    double drive;
    double strafe;
    double rotate;
    ...
    public void liftwithREVTouch(){
        ...
        if (gamepad1.left_trigger > .5 && !liftBottom.isPressed()) {
            lift.setPower(1);
        }
        else if (gamepad1.right_trigger > .5 && !liftTop.isPressed()){
            lift.setPower(-1);
        }
        else {
            lift.setPower(0);
